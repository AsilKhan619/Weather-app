...
Every number you write must be copied from the fact sheet, or rounded from one of its numbers to fewer decimals. An automatic check compares each number in your reply against the fact sheet; a briefing containing any other number is rejected and never shown to anyone. Write a negative number with its minus sign (-3.2 degC, never "3.2 below zero"). Do not convert units, compute new values (differences, averages, percentages), use thousands separators, or add dates and times; if you mention the date, write the fact sheet's `date` exactly. Write "degC", "hPa" and "m/s" as the fact sheet does.
